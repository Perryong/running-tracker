...
export const HeartRateTrendPanel = ({
  ...
  const zoneData = useMemo(() => {
    ...
    Object.keys(zoneAggregates).forEach((zone) => {
      zoneAggregates[zone].percentage =
        totalDuration > 0
          ? (zoneAggregates[zone].duration / totalDuration) * 100
          : 0;
    });
      ...
      <div className="mb-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button
            ...
            className="inline-flex h-6 w-6 items-center justify-center rounded transition-colors hover:bg-[var(--color-card-bg)]"
            aria-expanded={!isCollapsed}
            aria-label={
              isCollapsed
                ? 'Expand Heart Rate Trend'
                : 'Collapse Heart Rate Trend'
            }
          >
        ...
        <>
          {!zoneData ? (
            ...
            <div className="mb-2 rounded bg-[var(--color-card-bg)] p-4">
              <h3 className="mb-4 text-lg font-semibold text-[var(--color-brand)]">
              ...
              <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                ...
                <div data-testid="hr-zone-breakdown">
                  <h4 className="mb-3 font-semibold text-[var(--color-brand)]">
                    Zone Breakdown
                  </h4>
                  <div className="space-y-2">
                    {zoneData.map((zone) => (
                      <div
                        key={zone.name}
                        className="flex items-center justify-between rounded bg-[var(--color-bg)] p-2"
                      >
                  ...
                  <div className="border-[var(--color-hr-primary)]/20 mt-4 space-y-1 border-t pt-3 text-xs text-[var(--color-secondary)]">
                    <p>
                      <strong>Z1:</strong> 50-60% max HR - Recovery/Easy pace
                    </p>
                    <p>
                      <strong>Z2:</strong> 60-70% max HR - Aerobic endurance
                    </p>
                    <p>
                      <strong>Z3:</strong> 70-80% max HR - Tempo/Threshold
                    </p>
                    <p>
                      <strong>Z4:</strong> 80-90% max HR - Hard intervals
                    </p>
                    <p>
                      <strong>Z5:</strong> 90-100% max HR - Maximum effort
                    </p>
                  </div>
                </div>
              </div>
