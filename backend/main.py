...
Examples:
  ...
  Get Garmin secret:
    ...
    garmin_parser = subparsers.add_parser('sync-garmin', help='Sync activities from Garmin Connect')
    garmin_parser.add_argument('--secret', type=str, help='Garmin authentication secret')
    garmin_parser.add_argument('--is-cn', action='store_true', help='Use Garmin China domain')
    garmin_parser.add_argument('--only-run', action='store_true', help='Only sync running activities')
    garmin_parser.add_argument('--format', choices=['gpx', 'tcx', 'fit'], default='gpx', help='File format to download')
    garmin_parser.set_defaults(func=sync_garmin)
    
    svg_parser = subparsers.add_parser('generate-svg', help='Generate SVG visualizations')
    svg_parser.add_argument('--type', choices=['github', 'grid', 'circular', 'monthoflife'], 
                           default='github', help='Type of visualization')
    svg_parser.add_argument('--output', type=str, required=True, help='Output SVG file path')
    svg_parser.add_argument('--from-db', action='store_true', help='Load tracks from database')
    svg_parser.add_argument('--year', type=str, help='Filter by year (e.g., "2024" or "2020-2024")')
    svg_parser.add_argument('--title', type=str, help='Custom title for the visualization')
    svg_parser.add_argument('--language', type=str, help='Language for the visualization')
